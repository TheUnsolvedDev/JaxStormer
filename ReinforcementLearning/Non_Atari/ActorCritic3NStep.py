...
class ActorCritic:
    def __init__(self, env, num_actions, observation_shape, N_step=8, seed=0):
        self.seed = seed
    ...
    def calculate_n_step_return(self, rewards, gamma, n):
        T = len(rewards)
        n_step_returns = np.zeros(T)
    ...
    def train_single_step(self, reward_shape=0):
        ...
        for _ in range(500):
            ...
            if len(self.episode_dones) >= self.n_step or (truncated or done):
                new_rewards = jnp.array(self.calculate_n_step_return(
                    self.episode_rewards, GAMMA, self.n_step))

                episode_state = jnp.array(self.episode_states)
                episode_next_state = jnp.array(self.episode_next_states)
                episode_reward = new_rewards
...
if __name__ == '__main__':
    ...
    plot_data(mean_rcb, std_rcb, name='Cartpole AC N Step')
    ...
    plot_data(mean_rab, std_rab, name='Acrobot AC N Step')
