...
class ActorCritic:
    ...
    def policy_prob(self, q_state, state):
        probs = self.actor_critic.apply(q_state.params, state)[0]
        return probs
    ...
    @functools.partial(jax.jit, static_argnums=(0,))
    def update(self, actor_critic_state, states, actions, next_states, rewards, dones, gamma_t):

        @jax.jit
        def mse_loss_and_log_prob_loss(params):
            _, target = self.actor_critic.apply(params, next_states)
            target = rewards + GAMMA * target*(1.0-dones)
            probs, pred = self.actor_critic.apply(params, states)
    ...
    def train_single_step(self, reward_shape=0):
        ...
        for _ in range(500):
            ...
            episode_dones = jnp.array([done or truncated])
            gamma_t = GAMMA ** _

            loss, self.actor_critic_state = self.update(
                self.actor_critic_state, episode_state, episode_action, episode_next_state, episode_reward, episode_dones, gamma_t)
            total_loss += loss
            state = next_state
