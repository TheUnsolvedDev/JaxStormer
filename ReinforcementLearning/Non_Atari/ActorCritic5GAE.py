...
class ActorCritic:
    def __init__(self, env, num_actions, observation_shape, lambd=0.9, seed=0):
        self.seed = seed
    ...
    @functools.partial(jax.jit, static_argnums=(0,))
    def update(self, actor_state, critic_state, states, actions, next_states, rewards, returns, advantages, dones, gamma_t):
    ...
    def compute_advantages(self, states, rewards, dones):
        ...
        next_value = 0
        values = self.critic.apply(self.critic_state.params, states)
        for t in range(len(rewards)-1, 0, -1):
            delta = rewards[t] + (1 - dones[t]) * GAMMA * \
                next_value - values[t][0]
            advantages[t] = delta + (1 - dones[t]) * \
                GAMMA * self.lambd * advantages[(t+1) % len(states)]
    ...
    def train_single_step(self, reward_shape=0):
        ...
        for _ in range(500):
            ...
            self.episode_dones.append(done or truncated)
            self.episode_next_states.append(next_state)
            state = next_state
            if done or truncated:
                break

        episode_state = jnp.array(self.episode_states)
        episode_next_state = jnp.array(self.episode_next_states)
        episode_reward = jnp.array(self.episode_rewards)
        episode_action = jnp.array(self.episode_actions)
        episode_done = jnp.array(self.episode_dones)
        returns, advantages = self.compute_advantages(
            episode_state, episode_reward, episode_done)
        gamma_t = GAMMA ** _

        loss, self.actor_state, self.critic_state = self.update(
            self.actor_state, self.critic_state, episode_state, episode_action, episode_next_state, episode_reward, returns, advantages, episode_done, gamma_t)
        total_loss += loss
        self.step_storage()
...
if __name__ == '__main__':
    ...
    plot_data(mean_rcb, std_rcb, name='Cartpole AC GAE')
    ...
    plot_data(mean_rab, std_rab, name='Acrobot AC GAE')
