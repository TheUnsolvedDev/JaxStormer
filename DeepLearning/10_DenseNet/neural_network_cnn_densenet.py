...
class Dataset:
    def __init__(self, batch_size: int = 128) -> None:
        ...
        Args:
            batch_size (int, optional): The size of the mini-batches for training. Defaults to 128.
...
class DenseLayer(flax.linen.Module):
    ...
    def __call__(self, x: jnp.ndarray, train: bool = True) -> jnp.ndarray:
        """
        Apply the DenseLayer to the input tensor.
        ...
        Returns:
            jnp.ndarray: The output tensor after applying the DenseLayer.
        """
...
class DenseBlock(flax.linen.Module):
    ...
    def __call__(self, x: jnp.ndarray, train: bool = True) -> jnp.ndarray:
        """
        Apply the DenseBlock to the input tensor.
        ...
        Returns:
            jnp.ndarray: The output tensor after applying the DenseBlock.
        """
...
class TransitionLayer(flax.linen.Module):
    ...
    def __call__(self, x: jnp.ndarray, train: bool = True) -> jnp.ndarray:
        """
        Apply the TransitionLayer to the input tensor.
        ...
        Returns:
            jnp.ndarray: The output tensor after applying the TransitionLayer.
        """
        x = flax.linen.BatchNorm()(x, use_running_average=not train)
        x = self.act_fn(x)
        x = flax.linen.Conv(
            self.c_out, kernel_size=(1, 1),
            kernel_init=densenet_kernel_init,
            use_bias=False)(x)
...
class DenseNet(flax.linen.Module):
    ...
    def __call__(self, x: jnp.ndarray, train: bool = True) -> jnp.ndarray:
        """
        Apply the DenseNet to the input tensor.
        ...
        Returns:
            jnp.ndarray: The output tensor after applying the DenseNet.
        """
...
class ConvolutionalNeuralNetwork:
    def __init__(
        ...
        model: DenseNet = DenseNet(),  # type: ignore
        model_name: str = 'DenseNet'  # type: ignore
    ) -> None:
        """
        ...
        Args:
            ...
            model (DenseNet, optional): The neural network model. Defaults to DenseNet().
            model_name (str, optional): The name of the model. Defaults to 'DenseNet'.
    ...
    def eval_model(self, data_loader: DataLoader) -> float:
        ...
        for batch in data_loader:
            ...
            acc = self.eval_step(self.state, (batch_images, batch_labels))
            correct_class += acc * batch_size
            count += batch_size
