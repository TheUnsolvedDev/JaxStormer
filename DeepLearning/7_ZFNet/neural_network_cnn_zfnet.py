...
class ZFNet(flax.linen.Module):
    ...
    def __call__(self, x: jnp.ndarray, train: bool = True) -> jnp.ndarray:
        """
        Apply the ZFNet convolutional neural network to the input data.
...
class ConvolutionalNeuralNetwork:
    def __init__(
        ...
        model: ZFNet = ZFNet(),  # type: ignore
        model_name: str = 'ZFNet'  # type: ignore
    ) -> None:
        """
        ...
        Args:
            ...
            model (ZFNet, optional): The neural network model. Defaults to ZFNet().
            model_name (str, optional): The name of the model. Defaults to 'ZFNet'.
    ...
    def init_model(self, dummy_image: np.ndarray = np.ones([10, *IMG_SHAPE, 1])) -> None:
        ...
        self.init_params: flax.core.frozen_dict.FrozenDict = variables['params']
        self.init_batch_stats: flax.core.frozen_dict.FrozenDict = variables['batch_stats']
        self.state: Optional[TrainStateNew] = None
    ...
    def train_step(self, state: TrainStateNew, batch: Tuple[jnp.ndarray, jnp.ndarray]) -> Tuple[TrainStateNew, float, float]:
        """
        Train the model for one step.

        Args:
            state (TrainStateNew): The training state.
            batch (Tuple[jnp.ndarray, jnp.ndarray]): A tuple containing the batch of images and labels.

        Returns:
            Tuple[TrainStateNew, float, float]: A tuple containing the updated training state, the loss, and the accuracy.
        """
